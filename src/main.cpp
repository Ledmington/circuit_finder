...
int main(int argc, char* argv[]) {
	...
	for (int i{1}; i < argc; i++) {
		if (cf::cli::is_help_argument(argv[i])) {
			...
			if (i + 1 >= argc) {
				std::cerr << "The parameter '--bits' needs an integer, but "
							 "none was found."
						  << std::endl;
				return -1;
			}
			...
			if (i + 1 >= argc) {
				std::cerr << "The parameter '--operation' needs an operation "
							 "name, but "
							 "none was found."
						  << std::endl;
				return -1;
